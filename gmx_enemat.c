/*
 * This file is part of the GROMACS Version 5.1.2 molecular simulation package.
 *
...
#define DONTSKIP(cnt) (skip) ? ((cnt % skip) == 0) : TRUE
    ...
    FILE *fp = fopen("pairwise_energy.txt", "w+");
    ...
    if (bMeanEmtx)
    {
        ...
        for (i = 0; (i < ngroups); i++)
        {
            for (j = i; (j < ngroups); j++)
            {
                ...
                if ( i != j ) {fprintf(fp, "%d\t%d\t%f\n", i,j,emat[egTotal][i][j]);}
            }
        }
        fclose(fp);  
        if (bFree)
